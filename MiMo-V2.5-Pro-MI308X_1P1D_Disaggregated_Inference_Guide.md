...
## 2. MiMo-V2.5-Pro single node TP=8 test
...
## 3. MiMo-V2.5-Pro 1P1D w/ PD disaggreate

### 2.1 Launch docker 
```bash
sudo docker run -it --name mimo-v2.5-pro-prefill --shm-size 64g --privileged --network=host --ipc=host \
--device=/dev/kfd --device=/dev/dri \
--group-add video \
--cap-add=SYS_PTRACE \
--security-opt seccomp=unconfined \
...
```
```bash
sudo docker run -it --name mimo-v2.5-pro-decode --shm-size 64g --privileged --network=host --ipc=host \
--device=/dev/kfd --device=/dev/dri \
--group-add video \
--cap-add=SYS_PTRACE \
--security-opt seccomp=unconfined \
...
```bash
ssh -p 8737 mi308-ccs-aus-e06-01 hostname
```
```bash
...
```bash
ssh -p 8737 mi308-ccs-aus-e06-10 hostname
```
```bash
...
TOKEN_LIST=(8192)
output_tokens=1024 
concurrency_list=(1 16 128) # 定义并发列表
...
for input_tokens in "${TOKEN_LIST[@]}"; do
  for concurrency in "${concurrency_list[@]}"; do # 遍历并发列表
    ...
    python3 -m sglang.bench_serving \
        ...
        --pd-separated \
        2>&1 | tee "./benchmark_log/benchmark_decode_${input_tokens}_con${concurrency}_mtp.log"
...
Starting warmup with 32 sequences...
Warmup completed with 32 sequences. Starting main benchmark run...
...
Benchmark duration (s):                  454.31    
Total input tokens:                      2097152   
Total input text tokens:                 2097152   
Total generated tokens:                  131072    
Total generated tokens (retokenized):    131066    
Request throughput (req/s):              0.28      
Input token throughput (tok/s):          4616.11   
Output token throughput (tok/s):         288.51    
Peak output token throughput (tok/s):    386.00    
Peak concurrent requests:                33        
Total token throughput (tok/s):          4904.62   
Concurrency:                             29.07     
----------------End-to-End Latency----------------
Mean E2E Latency (ms):                   103161.51 
Median E2E Latency (ms):                 93002.21  
P90 E2E Latency (ms):                    141870.65 
P99 E2E Latency (ms):                    172785.46 
---------------Time to First Token----------------
Mean TTFT (ms):                          14439.15  
Median TTFT (ms):                        3499.76   
P99 TTFT (ms):                           85232.20  
-----Time per Output Token (excl. 1st token)------
Mean TPOT (ms):                          86.73     
Median TPOT (ms):                        87.10     
P99 TPOT (ms):                           87.78     
---------------Inter-Token Latency----------------
Mean ITL (ms):                           86.73     
Median ITL (ms):                         87.18     
