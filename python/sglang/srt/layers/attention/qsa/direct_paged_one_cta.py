...
def _sparse_gqa_chunk_prefill(
    ...
    row_limit = tl.minimum(
        TOPK, ((row_topk + BLOCK_N - 1) // BLOCK_N) * BLOCK_N
    )
    ...
    for start in range(0, row_limit, BLOCK_N):
        ...
        logical_valid = (
            (current < row_topk) & (logical >= 0) & (logical < visible)
        )
        ...
        physical_page = tl.load(
            page_table
            + safe_page_table_row * spt_m
            + logical_page * spt_n,
            mask=page_column_valid,
            other=-1,
        ).to(tl.int64)
        valid = (
            page_column_valid & (physical_page >= 0) & (physical_page < num_pages)
        )
        physical_token = physical_page * PAGE_SIZE + page_offset
        keys = tl.load(
            k
            + physical_token[None, :] * sk_n
            + offs_d[:, None] * sk_d,
            mask=valid[None, :],
            other=0.0,
        )
    ...
    output = tl.where(
        has_values[:, None], accumulator / normalizer[:, None], 0.0
    )
    tl.store(
        out
        + batch * so_m
