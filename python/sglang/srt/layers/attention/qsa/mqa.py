...
def _launch_triton_qsa_mqa_decode(
    ...
    *,
    ...
    """Launch direct-paged QSA decode with an optional clean output tail."""

    _validate_decode_inputs(q, k_cache, page_table, context_lens)
    if q.dtype not in (torch.bfloat16, torch.float16) or k_cache.dtype != q.dtype:
        raise ValueError(
            "Triton QSA decode requires matching BF16/FP16 Q and K cache; "
    ...
    if not batch or not max_model_len:
        return logits
    q = q.contiguous()
    page_table = page_table.to(device=q.device, dtype=torch.int32).contiguous()
    context_lens = context_lens.to(device=q.device, dtype=torch.int32).contiguous()
    block_keys = 32
    _triton_qsa_mqa_decode_kernel[
        (batch, triton.cdiv(max_model_len, block_keys))
    ](
        q,
        k_cache,
        page_table,
