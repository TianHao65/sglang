...
def _is_mi308x() -> bool:
    return bool(
        ...
        and torch.cuda.get_device_properties(0).gcnArchName.split(":", 1)[0]
        == "gfx942"
...
def _fill_indices(indices: torch.Tensor, lengths: list[int], seed: int) -> None:
    ...
    for row, length in enumerate(lengths):
        ...
        if selected_groups:
            ...
            expanded = (
                groups[:, None] * COMPRESS_RATIO + offsets[None, :]
            ).reshape(-1)
        else:
...
def _make_backend_state(batch: int, lengths: list[int], seed: int, layers: int = 1):
    ...
    page_table = torch.randperm(
        num_pages, dtype=torch.int64, device=device
    ).view(batch, pages_per_request).to(torch.int32)
    row_to_page_table = torch.randperm(
...
def _update_state(state, lengths: list[int], seed: int) -> None:
    ...
    for layer_id in state.pool.keys:
        ...
        state.pool.values[layer_id].copy_(
            torch.randn_like(state.pool.values[layer_id])
        )
    ...
    state.row_to_page_table.copy_(
        torch.randperm(
            state.q.shape[0], dtype=torch.int64, device=state.q.device
        ).to(torch.int32)
    )
...
def _reference(state, layer_id: int) -> torch.Tensor:
    ...
    for row in range(state.q.shape[0]):
        ...
        physical_pages = state.page_table[page_row].long().index_select(
            0, page_columns[in_table]
        )
...
def _packed_inputs(state, layer_id: int):
    ...
    cu_q = torch.arange(
        state.q.shape[0] + 1, dtype=torch.int32, device=state.q.device
    )
...
def test_qsa_packed_one_cta_mi308x_launch_config(monkeypatch):
    fallback = (32, 8, 2, {})
    monkeypatch.setattr(
        sparse_module, "_get_best_config", lambda total_q: fallback[:3]
    )
    ...
    assert (
        sparse_module._get_sparse_gqa_packed_decode_launch_config(
            *target, *dtypes
        )
        == fallback
    )
...
def test_qsa_same_named_chunk_prefill_kernels_do_not_collide_in_one_process():
    ...
    packed_k, packed_v, relative_indices, cu_q, cu_k, kv_lens = _packed_inputs(
        state, 0
    )
    ...
    assert not any(
        "splitk" in name.lower() or "merge" in name.lower()
...
def test_qsa_fast_topk_expand_dispatches_direct_paged_one_cta(monkeypatch):
    ...
    assert (
        sum(
            "_expand_qsa_block_indices_kernel" in name
            for name in gpu_kernel_names
        )
        == 1
    )
